...
#IFNDEF TOP
    ...
    cFiltro :="F3_FILIAL=='"+xFilial("SF3")+"'.AND. DTOS(F3_ENTRADA)>='"+DTOS(dDtIni)+"'.AND. DTOS(F3_ENTRADA)<='"+DTOS(dDtFim)+"'.AND. "+SerieNfId(cAliasSF3,3,'F3_SERIE')+"=='"+cSerieT+"'"
...
cChave:="F3_FILIAL+DTOS(F3_ENTRADA)+"+SerieNfId(cAliasSF3,3,'F3_SERIE')
...
If !(Bof() .And. Eof())  //Verifica se ha faturas para a impressao
   ...
   While xFilial("SF3") == TRB2->F3_FILIAL .And. !Eof()
      If Empty(TRB2->F3_DTCANC)              
         ...
         @ nLin,014 PSAY SerieNfId('TRB2',2,'F3_SERIE')
