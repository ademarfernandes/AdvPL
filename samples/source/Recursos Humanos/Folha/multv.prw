...
Begin Sequence
	...
	While !SRA->(Eof()) .and. SRA->RA_CIC <= cCpfAte_
		...
		For nCateg:= 1 To Len(aCateg)   
			...
			If Type("aInssOut") == "A" .And. Len(aInssOut) > 0 .And. aInssOut[1][5] <> SRA->RA_CIC
				aInssOut:= {}
			EndIf
...
Else
	If aX[3] == cSemana
		...
		GravaSrc(SRA->RA_FILIAL,SRA->RA_MAT,aX[1],If(!Empty(aX[10]),aX[10],dData_Pgto),cCTC,;
				If(Empty(aX[3]),cSemana,aX[3] ),aX[6],aX[7],aX[4],aX[5],aX[8],aX[9],,aX[11],aX[12],aX[13],aX[14],If(Len(aX)>= 17,aX[17],''),aX[20],aX[21],aX[15],aX[18],aX[22])
	EndIf
EndIf
Return( NIL )
		...
		aPerAtual 	:= {}
	   	...
	   	Else 		   	
			...
			If Len(aPerAtual) > 0
				...
				cSemana	:= cSemAux 	:= aPerAtual[1,2] //Numero de pagamento atual
			Else
				If SRA->RA_CATFUNC $ "A*P" .And. cTipoRot <> "9"
					SRA->( DbSkip() )
					Loop
				Else
					CMSGLOG := FMSGFORM({}) + "O PERIODO '" + CPERIODO + "' NÃO ESTÁ ATIVO. SELECIONE UM PERIODO ATIVO PARA O CÁLCULO. PROCESSO -> " + SRA->RA_PROCES + ". FILIAL -> " + SRA->RA_FILIAL
	...
	For ny := 1 to Len(aCPF)
		...
		If aCPF[ny,8]
			...
			If aCPF[ny,7] <> "2" // somente para funcionarios acumulados em APDTOT
				...
				If Type("nAliqFMV") <> "U" .And. nAliqFMV > 0
					nBsINSSFER := 0
						...
						//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
							...
							If (aCPF[ny,4] <> "5") .AND. (cCodPD != aCodFol[034,1])
								...
								aAdd(&(cNomeArray),{	cCodPd, cCCAtual, cSemAux, aPdTot[nPosTot,4],		;
														...
														aPdTot[nPosTot,17],aPdTot[nPosTot,18],aPdTot[nPosTot,19],aPdTot[nPosTot,20],aPdTot[nPosTot,21],aPdTot[nPosTot,22] } )
								nPos := Len(&(cNomeArray))
							EndIf						
						EndIf
						//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
					...
					If aTroca[nx,3] == "1"
                        ...
                        If nValTot == 0 .And. aCodFol[064,1] $ aTroca[nx,1]
                        	...
                        	If nPos > 0 .And. (&(cNomeArray)[nPos,7] $"I,G")
								...
								EndIf
								 ...
								 If nTotAux > 0 .And. Type("aInssOut") == "A" 
								   	If Len(aInssOut) > 0
                        ...
                        If nValTot > 0
							...
							ElseIf nPos > 0 .and. (&(cNomeArray)[nPos,7] $ "I,G") //Busca Verbas de INSS Outra Empresa caso sejam inFormadas
								    ...
								    If Type("aInssOut") == "A" .And. Len(aInssOut) > 0 .And. aScan(aInssOut,{|x| x[3] == aCpF[nY,2] .And. x[4] == aCpF[nY,3]}) >  0
										cOrigem:= aPdTot[nPosTot,7]
								 	Else
								  		cOrigem:= "C"
								 	EndIf
								    
								    IF Type("aInssOut") == "A" .And. Len(aInssOut) > 0 .And. aScan(aInssOut,{|x| x[3] == aCpF[nY,2] .And. x[4] == aCpF[nY,3]}) >  0 .And. cOrigem == "C"
								      	cSeq:= "2"
								 	Else
							...
							ElseIf nPos == 0  
						   		...
						   		If Type("aInssOut") == "A" .And. Len(aInssOut) > 0  .And. If( cTipoRot == "6", nValTot <> (aTInss13[Len(aTInss13)][1]*aTInss13[Len(aTInss13)][2] ) , nValTot <> (aTInss[Len(aTInss)][1]*aTInss[Len(aTInss)][2] ) )
								...
								If aCPF[ny,4] <> "5"  .OR. (nProc1 == 1 .and. aCPF[ny,5] == "A" .and. aCPF[ny,4] == "5" ) //se For demitido OU novo processamento inclue verbas
									...
									Else
										If Type("aInssOut") == "A" .And. Len(aInssOut) > 0 .And. nPos > 0 .And. aScan(aInssOut,{|x| x[3] == &(cNomeArray)[nPos,2] .And. x[4] == &(cNomeArray)[nPos,3]}) >  0
											cOrigem:= aPdTot[nPosTot,7]
									 	Else
									  		cOrigem:= "C"
									 	EndIf
...
GravaSrc(SRA->RA_FILIAL,SRA->RA_MAT,aX[1], dDtPgt ,aX[2],aX[3],aX[6],aX[7],aX[4],aX[5],aX[8],ax[9],,aX[11],,,,,aX[20],aX[21],aX[15],aX[18],aX[22])
		...
		Else
			...
			While SRC->( !Eof() .And. SRA->( RA_FILIAL + RA_MAT + RA_PROCES ) + cRotAux + cPeriodo + cSemana == RC_FILIAL + RC_MAT + RC_PROCES + cRotAux + RC_PERIODO + RC_SEMANA )
				If SRC->RC_ROTEIR $ cRotAux
					...
					SRC->( FMatriz(RC_PD,RC_VALOR,RC_HORAS,RC_SEMANA,RC_CC,RC_TIPO1,RC_TIPO2,RC_PARCELA,cDel,dDtapg,,RC_SEQ,RC_QTDSEM,RC_ITEM, RC_CLVL,,,,RC_DTREF ) )
					dbSelectArea("SRC")
				EndIf
				dbSkip()
			EndDo
...
Else
	If lItemClVl
		GravaSrc(SRA->RA_FILIAL,SRA->RA_MAT,aX[1],If(!Empty(aX[10]),aX[10],dData_Pgto),aX[2],;
			cSemana,aX[6],aX[7],aX[4],aX[5],aX[8],aX[9],,aX[11],aX[12],aX[13],aX[14],If(Len(aX)>= 17,aX[17],''),aX[20],aX[21],aX[15],aX[18],aX[22])
	Else
		GravaSrc(SRA->RA_FILIAL,SRA->RA_MAT,aX[1],If(!eMPTY(aX[10]),aX[10],dData_Pgto),aX[2],;
			cSemana,aX[6],aX[7],aX[4],aX[5],aX[8],aX[9], ,aX[11],aX[12],,,If(Len(aX)>= 17,aX[17],''),aX[20],aX[21],aX[15],aX[18],aX[22])
	EndIf		
EndIf		
